...
import '../components/common/source_selection_modal.dart';
...
class _FormerToVendorScreenState extends State<FormerToVendorScreen> {
  ...
  SourceSelectionResult? _selectedSource;
  ...
  // Machine & Line Selection (for fromProduction)
  ...
  Future<void> _showSourceSelectionModal() async {
    ...
    final source = await SourceSelectionModal.show(context);
  ...
  void _handleTagScanned(TagData tagData) {
    ...
    if (!_isValidRfidTag(tagId)) return;
    if (_allRackTagIds.contains(tagId)) return;
    if (_scannedItemsMap.containsKey(tagId) || _pendingTags.containsKey(tagId)) return;
  ...
  Future<void> _processBatchQueue() async {
    ...
    try {
      ...
      if (_selectedSource?.isFromProduction == true) {
        baskets = await ApiService.getBasketsStockInBatch(batchIds, warehouse: _warehouseCode);
      } else {
        baskets = await ApiService.getBasketsStockOutBatch(batchIds, warehouse: _warehouseCode);
      }
      ...
      _isProcessingBatch = false;
      if (_pendingTags.isNotEmpty) _resetBatchTimer();
    }
  }
  ...
  Future<void> _handleSingleFilledScan(TagData tagData) async {
    ...
    try {
      List<BasketData> raw = [];
      if (_selectedSource?.isFromProduction == true) {
        raw = await ApiService.getBasketsStockInBatch([tagData.tagId], warehouse: _warehouseCode);
      } else {
        raw = await ApiService.getBasketsStockOutBatch([tagData.tagId], warehouse: _warehouseCode);
      }
  ...
  Future<void> _startScanning() async {
    ...
    if (_basketMode == BasketMode.filled) _singleTagCaptured = false;
  ...
  Future<void> _restoreRackCache() async {
    ...
    final restoredRacks = (decoded['racks'] as List)
        ...
        .toList();
    final restoredTagIds = Set<String>.from(decoded['allRackTagIds'] ?? const []);
  ...
  String get _effectiveStockForm {
    if (_selectedSource?.isFromProduction == true && _selectedStockoutForm != null) {
          ...
          SingleChildScrollView(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  if (_selectedSource!.isFromProduction) ...[
