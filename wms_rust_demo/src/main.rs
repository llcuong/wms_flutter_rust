...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route("/api/v2/baskets/stockin_batch", post(handle_stockin_batch_baskets))
...
async fn handle_stockin_batch_baskets(
    ...
    tracing::info!("🔍 SQL Query (StockIn): {}", query.replace("\n", " ").trim());
            ...
            match stream.into_first_result().await {
                Ok(rows) => {
                    ...
                    for row in rows {
                        ...
                        let status_str = if is_active_val == 1 { "Active" } else { "Inactive" };

                        let basket = BasketData {
                            tag_id: row.get::<&str, _>("basket_no").unwrap_or_default().to_string(),
                            basket_vendor: row.get::<&str, _>("basket_vendor").map(|s| s.to_string()),
                            basket_purchase_order: row.get::<&str, _>("basket_purchase_order").map(|s| s.to_string()),
...
async fn create_db_pool() -> Result<Pool<ConnectionManager>, Box<dyn std::error::Error>> {
    ...
    tracing::info!("📦 Connecting to SQL Server: {}:{}/{}", host, port, database);
...
async fn handle_get_parameters(
    ...
    let (query, param) = match group {
        ...
        "vendor" | "brand" => ( // Map 'brand' request to 'vendor' query logic if needed, or stick to strict group name
            r#"
            SELECT code, name 
            FROM [VNWMS].[dbo].[wh_former_parameter_data] 
    ...
    tracing::info!("✅ Found {} parameter options for group '{}'", results.len(), group);
            ...
            match stream.into_first_result().await {
                Ok(rows) => {
                    if let Some(row) = rows.first() {
                        ...
                        tracing::warn!("⚠️ Item No not found or inactive: {}. Defaulting to 1.", item_no);
        ...
        Ok(rows) => {
            ...
            for row in rows {
                data.push(MachineData {
                    area_id: row.get::<&str, _>("area_id").unwrap_or_default().to_string(),
...
async fn handle_get_stockout_forms(
    ...
    tracing::info!("📥 Fetching stockout forms for machine: {}, line: {}", machine, line);
    ...
    match stream.into_first_result().await {
        Ok(rows) => {
            ...
            for row in rows {
                ...
                let date_str = row.try_get::<NaiveDate, _>("stockout_date")
                    .ok()
                    .flatten()
                    .map(|d| d.to_string())
                    .or_else(|| row.try_get::<&str, _>("stockout_date").ok().flatten().map(|s| s.to_string()));
                ...
                data.push(StockoutFormData {
                    id: get_int("id"),
                    stockout_form: row.get::<&str, _>("stockout_form").unwrap_or("").to_string(),
...
async fn handle_stockin_save(
    ...
    tracing::info!("📦 Stock In Save request: form={}, machine={}", payload.stockin_form, payload.selected_machine);

    // Calculate totals
    let total_baskets: i32 = payload.racks.iter().map(|r| r.items.len() as i32).sum();
    let total_formers: i32 = payload.racks.iter()
        .flat_map(|r| r.items.iter())
        .map(|i| i.basket_former_qty)
        .sum();

    tracing::info!("📊 Totals: {} baskets, {} formers", total_baskets, total_formers);

    // Debug log each item
    for rack in &payload.racks {
        tracing::info!("  Rack {}: bin={}", rack.rack_no, rack.bin);
        for item in &rack.items {
            tracing::info!("    Item: basket_no={}, qty={}", item.basket_no, item.basket_former_qty);
        }
    }
    ...
    let (batch_no, used_day): (String, i32) = {
        ...
        let stream = match conn.query(query_get_form, &[
        ...
        let most_used_day: i32 = row.try_get::<i32, _>("most_batch_used_day")
            .ok().flatten().unwrap_or(0);
        
        // Calculate used_day = most_batch_used_day + (today - stockout_date).days + 10
        let stockout_date = row.try_get::<chrono::NaiveDate, _>("stockout_date")
            .ok().flatten();
    ...
    let query_update_form = r#"
        ...
        WHERE stockout_form = @P3
          ...
          AND stockout_to = @P5
    "#;
    let _ = conn.execute(query_update_form, &[
        &total_baskets,
        &total_formers,
        &payload.stockin_form,
        &payload.former_size,
    ...
    for rack in &payload.racks {
        for item in &rack.items {
            ...
            let query_basket = r#"
                ...
                WHERE basket_no = @P3;
            "#;
            match conn.execute(query_basket, &[
                &used_day,
                &payload.former_size,
                &item.basket_no,
            ]).await {
                Ok(result) => tracing::info!("✅ basket_master_data updated: {} rows for basket_no={}", result.total(), item.basket_no),
            ...
            let query_bin = r#"
                ...
                WHEN NOT MATCHED THEN
                    ...
                    VALUES (@P1, @P2, @P3, @P4, '', GETDATE());
            "#;
            match conn.execute(query_bin, &[
                &item.basket_no,
                &rack.bin,
                &item.basket_former_qty,
                &batch_no,
            ]).await {
                Ok(result) => tracing::info!("✅ bin_data upserted: {} rows, bin={}", result.total(), rack.bin),
            ...
            "#;
            match conn.execute(query_bin_log, &[
                &batch_no,
                &item.basket_no,
    ...
    if log_exists {
        ...
        "#;
        let _ = conn.execute(query_update_log, &[
            &total_formers,
            &total_baskets,
            &batch_no,
            &key,
        ...
        "#;
        let _ = conn.execute(query_insert_log, &[
            &batch_no,
            &key,
            &total_formers,
            &total_baskets,
    ...
    let query_update_batch = r#"
        ...
        WHERE batch_no = @P4
    "#;
    let _ = conn.execute(query_update_batch, &[
        &total_baskets,
        &total_formers,
        &used_day,
        &batch_no,
